...
/* --------------------- hidden tests (update after ddl) -------------------- */

class RTP_HIDDEN : public ::testing::Test {
    void SetUp() override {
        std::snprintf(origin, 100, "%s%d", ORIGIN, rand() % 12345 + 123);
        std::snprintf(result, 100, "%s%d", RESULT, rand() % 12345 + 123);
        create_random(5);
